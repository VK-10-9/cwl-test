      ...
      {DOCUMENT_TYPES.map((type) => {
        ...
        return (
          <Link key={type} href={`/generate/${type}`} className="group block focus:outline-none focus:ring-2 focus:ring-ring rounded-xl">
            <Card className="h-full flex flex-col bg-card border-border hover:border-primary/40 transition-all duration-200 hover:-translate-y-0.5">
              <CardHeader>
                <div className="w-11 h-11 rounded-lg bg-primary/10 flex items-center justify-center text-primary mb-3 group-hover:scale-105 transition-transform duration-200">
                  <Icon size={22} />
                </div>
              ...
              {/* Hover Indicator */}
              <div className="px-6 pb-5 pt-0 opacity-0 group-hover:opacity-100 transition-opacity duration-200 text-primary text-sm font-medium flex items-center gap-2">
                Start Drafting →
              </div>
            </Card>
          </Link>
