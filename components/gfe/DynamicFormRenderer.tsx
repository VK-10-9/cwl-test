  ...
  return (
    <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        {formDef.fields.map((field) => (
          <div key={field.key} className={`${field.type === "textarea" || field.type === "address" ? "col-span-full" : ""}`}>
            ...
            <div className="relative">
              ...
              {field.type === "textarea" || field.type === "address" ? (
                <textarea
                  ...
                  className="w-full pl-10 pr-4 py-3 bg-background/50 border border-border/60 text-foreground placeholder:text-muted-foreground rounded-xl focus:outline-none focus:ring-2 focus:ring-primary/50 focus:border-primary/50 transition-all shadow-sm min-h-[100px]"
                />
              ) : field.type === "date" ? (
                <input
                  type="date"
                  {...register(field.key, { required: field.required })}
                  className="w-full pl-10 pr-4 py-3 bg-background/50 border border-border/60 text-foreground placeholder:text-muted-foreground rounded-xl focus:outline-none focus:ring-2 focus:ring-primary/50 focus:border-primary/50 transition-all shadow-sm"
                />
              ) : (
                <input
                  type="text"
            ...
            {errors[field.key] && (
              <p className="text-primary/80 text-xs mt-2 font-medium flex items-center">
                <span className="w-1 h-1 rounded-full bg-primary mr-1.5 flex-shrink-0"></span>
      ...
      <div className="pt-8 border-t border-border/40 flex justify-end">
        <button
          type="submit"
          className="px-6 py-3 bg-primary/80 hover:bg-primary text-white font-medium rounded-xl shadow-sm focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-offset-background border border-primary/20 focus:ring-primary transition-all"
        >
